...
import com.newbee.mall.common.*;
import com.newbee.mall.controller.vo.*;
import com.newbee.mall.dao.MallGoodsMapper;
import com.newbee.mall.dao.MallOrderItemMapper;
import com.newbee.mall.dao.MallOrderMapper;
import com.newbee.mall.dao.MallShoppingCartItemMapper;
import com.newbee.mall.entity.NewBeeMallGoods;
import com.newbee.mall.entity.NewBeeMallOrder;
import com.newbee.mall.entity.NewBeeMallOrderItem;
import com.newbee.mall.entity.StockNumDTO;
import com.newbee.mall.service.NewBeeMallOrderService;
import com.newbee.mall.util.BeanUtil;
import com.newbee.mall.util.NumberUtil;
...
public class NewBeeMallOrderServiceImpl implements NewBeeMallOrderService {
    ...
    public PageResult getNewBeeMallOrdersPage(PageQueryUtil pageUtil) {
        List<NewBeeMallOrder> newBeeMallOrders = mallOrderMapper.findNewBeeMallOrderList(pageUtil);
    ...
    public String updateOrderInfo(NewBeeMallOrder newBeeMallOrder) {
        NewBeeMallOrder temp = mallOrderMapper.selectByPrimaryKey(newBeeMallOrder.getOrderId());
    ...
    public String checkDone(Long[] ids) {
        ...
        List<NewBeeMallOrder> orders = mallOrderMapper.selectByPrimaryKeys(Arrays.asList(ids));
    ...
    public String closeOrder(Long[] ids) {
        ...
        if (!CollectionUtils.isEmpty(orders)) {
            ...
            if (StringUtils.isEmpty(errorOrderNos)) {
                ...
                if (mallOrderMapper.closeOrder(Arrays.asList(ids), MallOrderStatusEnum.CLOSED_BY_JUDGE.getOrderStatus()) > 0) {
                    return ServiceResultEnum.SUCCESS.getResult();
                } else {
                    return ServiceResultEnum.DB_ERROR.getResult();
                }
    ...
    public String saveOrder(NewBeeMallUserVO user, List<NewBeeMallShoppingCartItemVO> myShoppingCartItems) {
        ...
        List<NewBeeMallGoods> newBeeMallGoods = mallGoodsMapper.selectByPrimaryKeys(goodsIds);
        //检查是否包含已下架商品
        ...
        if (!CollectionUtils.isEmpty(itemIdList) && !CollectionUtils.isEmpty(goodsIds) && !CollectionUtils.isEmpty(newBeeMallGoods)) {
            if (mallShoppingCartItemMapper.deleteBatch(itemIdList) > 0) {
                ...
                //生成订单项并保存订单项纪录
                if (mallOrderMapper.insertSelective(newBeeMallOrder) > 0) {
                    //生成所有的订单项快照，并保存至数据库
    ...
    public NewBeeMallOrderDetailVO getOrderDetailByOrderNo(String orderNo, Long userId) {
        NewBeeMallOrder newBeeMallOrder = mallOrderMapper.selectByOrderNo(orderNo);
        if (newBeeMallOrder != null) {
            //todo 验证是否是当前userId下的订单，否则报错
            List<NewBeeMallOrderItem> orderItems = mallOrderItemMapper.selectByOrderId(newBeeMallOrder.getOrderId());
            //获取订单项数据
    ...
    public PageResult getMyOrders(PageQueryUtil pageUtil) {
        ...
        if (total > 0) {
            ...
            if (!CollectionUtils.isEmpty(orderIds)) {
                List<NewBeeMallOrderItem> orderItems = mallOrderItemMapper.selectByOrderIds(orderIds);
    ...
    public String cancelOrder(String orderNo, Long userId) {
        ...
        if (newBeeMallOrder != null) {
            //todo 验证是否是当前userId下的订单，否则报错
            //todo 订单状态判断
            if (mallOrderMapper.closeOrder(Collections.singletonList(newBeeMallOrder.getOrderId()), MallOrderStatusEnum.CLOSED_BY_MANUAL.getOrderStatus()) > 0) {
                return ServiceResultEnum.SUCCESS.getResult();
            } else {
                return ServiceResultEnum.DB_ERROR.getResult();
            }
    ...
    public String finishOrder(String orderNo, Long userId) {
        ...
        if (newBeeMallOrder != null) {
            //todo 验证是否是当前userId下的订单，否则报错
            //todo 订单状态判断
            newBeeMallOrder.setOrderStatus((byte) MallOrderStatusEnum.SUCCESS.getOrderStatus());
    ...
    public String paySuccess(String orderNo, int payType) {
        ...
        if (newBeeMallOrder != null) {
            ...
            newBeeMallOrder.setOrderStatus((byte) MallOrderStatusEnum.PAID.getOrderStatus());
    ...
    public List<NewBeeMallOrderItemVO> getOrderItems(Long id) {
        NewBeeMallOrder newBeeMallOrder = mallOrderMapper.selectByPrimaryKey(id);
        if (newBeeMallOrder != null) {
            List<NewBeeMallOrderItem> orderItems = mallOrderItemMapper.selectByOrderId(newBeeMallOrder.getOrderId());
            //获取订单项数据
            if (!CollectionUtils.isEmpty(orderItems)) {
