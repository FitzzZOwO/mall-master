...
<template>
  <div class="order-detail-box">
    ...
    <div class="order-status">
      ...
      <van-button @click="showPayFn" block color="#1baeae" style="margin-bottom: 10px" v-if="detail.orderStatus == 0">
        去支付
      </van-button>
      <van-button @click="handleCancelOrder(detail.orderNo)" block
                  v-if="!(detail.orderStatus < 0 || detail.orderStatus == 4)">取消订单
      </van-button>
    </div>
    ...
    >
      <div :style="{ width: '90%', margin: '0 auto', padding: '20px 0' }">
        <van-button :style="{ marginBottom: '10px' }" @click="handlePayOrder(detail.orderNo, 1)" block color="#1989fa">
          支付宝支付
        </van-button>
        <van-button @click="handlePayOrder(detail.orderNo, 2)" block color="#4fc08d">微信支付</van-button>
      </div>
    </van-popup>
  </div>
</template>
