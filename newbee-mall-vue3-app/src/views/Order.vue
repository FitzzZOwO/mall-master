...
<template>
  <div class="order-box">
    <s-header :back="'/user'" :name="'我的订单'"></s-header>
    <van-tabs :color="'#1baeae'" :title-active-color="'#1baeae'" @click="onChangeTab" class="order-tab"
              v-model="status">
    ...
    <div class="content">
      <van-pull-refresh @refresh="onRefresh" class="order-list-refresh" v-model="refreshing">
        <van-list
                :finished="finished"
                @load="onLoad"
                @offset="10"
        ...
        >
          <div :key="index" @click="goTo(item.orderNo)" class="order-item-box" v-for="(item, index) in list">
